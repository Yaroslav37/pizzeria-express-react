...
const getAllSortedBy = async (sortBy = "id", sortDirection = "ASC") => {
  const orders = await database.query(
    `SELECT orders.id as id, json_agg(DISTINCT(users)) as user,  json_agg(products) as products, sum(products.price) as price, orders.created_at as created_at FROM orders
